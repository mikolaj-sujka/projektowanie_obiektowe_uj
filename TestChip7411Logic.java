...
public class TestChip7411Logic {

    @Test
    public void chip7411ComponentLogic() throws UnknownChip, UnknownPin, ShortCircuitException, UnknownComponent, UnknownStateException, UnknownStateException {
        Simulation simulation = new Simulation();

        int inputStrip = simulation.createInputPinHeader(9);
        int chip = simulation.createChip(7411);
        int outputStrip = simulation.createOutputPinHeader(3);

        // Wejścia dla każdej bramki AND
        ...
        simulation.connect(inputStrip, 4, chip, 3);  // A2
        simulation.connect(inputStrip, 5, chip, 4);  // B2
        simulation.connect(inputStrip, 6, chip, 5);  // C2
    ...
    private void tableForChip7411(Map<Integer, Set<ComponentPinState>> states0List) {
